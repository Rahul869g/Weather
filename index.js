const API_KEY = "267460f1a7a7526df85efc00da33c66e";
...
const loadCurrentForecast = ({
  ...
  const currentForecastElement = document.querySelector("#current__forecast");
...
const loadHourlyForecast = (
  ...
  const hourly_containerEl = document.querySelector(".hourly_container");
  let innerHtmlString = `<article>
                            <h3 class="time">Now</h3>
                            <img class="icon" src="${createIconUrl(
                              iconNow
                            )}" alt="" />
                            <p class="hourly-temp">${formatTemperature(
                              tempNow
                            )}</p>
                          </article>`;
  ...
  hourly_containerEl.innerHTML = innerHtmlString;
};
    ...
    ([day, { temp_max, temp_min, icon }], index) => {
      if (index < 5) {
        dayWiseInfo += `<article class="day_wise_forecast">
                                    ...
                                    <p class="min-temp">${formatTemperature(
                                      temp_min
                                    )}</p>
                                    <p class="max-temp">${formatTemperature(
                                      temp_max
                                    )}</p>
                                  </article>`;
      }
...
const loadFeelsLike = ({ main: { feels_like } }) => {
  let feelsLikeContainer = document.querySelector("#feels__like");
...
const onSearchChange = async (event) => {
  ...
  if (value && selectedCityText !== value) {
    const lisyOfCities = await getCitiesUsingGeoLocation(value);
    let options = ``;
    for (let { lat, lon, name, state, country } of lisyOfCities) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  serachInput.addEventListener("input", onSearchChange);
